...
let nextItemTime = 0; // 次の「通常」アイテム生成時刻 (ms)
let nextSoftServeTime = 0; // ★★★ 次の「ソフトクリーム」生成試行時刻 (ms) ★★★
...
let totalSoftServeToSpawn = 0;
let softServeSpawnedCount = 0;
...
export async function initializeGame(timeLimit) {
  ...
  softServeSpawnedCount = 0; // カウンターリセットは startGameLoop で行う
  console.log(
    `[Game] Time: ${currentGameTimeLimit}s, Total Soft Serves: ${totalSoftServeToSpawn}`
  );

  try {
    // ... (OpenCV Ready, Image Loading) ...
    ...
    // ... (Cascade, Camera, CV Objects Init) ...
    ...
    // ... (Score, Opacity, Instance Arrays Reset) ...
    currentScore = 0;
...
function startGameLoop() {
  ...
  console.log(
    `[startGameLoop] Soft Serve Count Reset: ${softServeSpawnedCount}/${totalSoftServeToSpawn}`
  );
  ...
  // ★★★ ソフトクリームの最初の出現試行時刻をランダムに設定 (例: 5-15秒後) ★★★
  const firstSoftServeDelay = (Math.random() * 10 + 5) * 1000; // 5000ms to 15000ms
  nextSoftServeTime = gameStartTime + firstSoftServeDelay;
  console.log(
    `[startGameLoop] Initial nextItemTime: ${nextItemTime.toFixed(
      0
    )}, Initial nextSoftServeTime: ${nextSoftServeTime.toFixed(0)}`
  );
...
function updateAndDrawItems(timestamp, elapsedTimeInSeconds, dt) {
  let regularItemGenerated = false; // 通常アイテム生成フラグ
  ...
  if (timestamp >= nextItemTime) {
    ...
    if (regularItemGenerated) {
      ...
      console.log(
        `[Game ItemGen] --- ${generatedItemType} generated! New next regular: ${nextItemTime.toFixed(
          0
        )}`
      );
    } else {
      nextItemTime = timestamp + 150; // スキップ時
    }
  } // --- 通常アイテム生成判定ここまで ---

  // ★★★ ソフトクリーム生成判定 (通常アイテムとは別のタイミング管理) ★★★
  if (
    timestamp >= nextSoftServeTime && // 次の出現試行時刻を過ぎたか？
    softServeSpawnedCount < totalSoftServeToSpawn && // まだ総数に達していないか？
    softServeInstances.length === 0
  ) {
    // 現在画面上にソフトクリームがないか？
    console.log(
      `%c[ItemGen] Time condition met for Soft Serve! (timestamp: ${timestamp.toFixed(
        0
      )} >= next: ${nextSoftServeTime.toFixed(0)})`,
      "color: magenta;"
    );
    ...
    // 次のソフトクリーム出現試行時刻を設定
    if (softServeSpawnedCount < totalSoftServeToSpawn) {
      // 例: 10秒後〜25秒後のランダムなタイミング
      const nextDelay = (Math.random() * 15 + 10) * 1000;
      nextSoftServeTime = timestamp + nextDelay;
      console.log(
        `[ItemGen] Next Soft Serve attempt scheduled at ${nextSoftServeTime.toFixed(
          0
        )} (in ${nextDelay / 1000}s)`
      );
    } else {
      nextSoftServeTime = Infinity; // もう出現しない
      console.log("[ItemGen] All Soft Serves have been spawned.");
    }
  }

  // --- 既存アイテムの更新と描画 (全種類、dt を渡す) ---
