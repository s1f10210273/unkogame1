...
window.handleOpenCvReady = () => {
    ...
    if (!window.cv) {
        ...
        if (game.getCurrentGameState() !== GAME_STATE.PLAYING && game.getCurrentGameState() !== GAME_STATE.COUNTDOWN) {
            game.setGameState(GAME_STATE.ERROR);
            ui.updateButtonState(game.getCurrentGameState());
        }
        return;
    }
    cvUtils.setCvReady() // Assuming setCvReady returns a Promise now
        .then(() => {
            console.log("OpenCV Runtime is confirmed ready.");
            if (game.getCurrentGameState() === GAME_STATE.IDLE || game.getCurrentGameState() === GAME_STATE.ERROR) {
...
window.handleOpenCvError = () => {
    ...
    ui.showLoadingMessage('OpenCV.js のロードに失敗しました。ネットワーク接続を確認してください。', true);
    if (game.getCurrentGameState() !== GAME_STATE.PLAYING && game.getCurrentGameState() !== GAME_STATE.COUNTDOWN) {
         game.setGameState(GAME_STATE.ERROR);
         ui.updateButtonState(game.getCurrentGameState());
    }
