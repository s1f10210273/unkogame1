...
export async function loadFaceCascade() {
    ...
    try {
        ...
        if (!faceCascade.load(FACE_CASCADE_FILE)) {
             throw new Error(`Error loading face cascade from OpenCV FS path /${FACE_CASCADE_FILE}.`);
        }
        ...
        faceCascade = null;
        throw new Error(`カスケードファイル (${FACE_CASCADE_PATH}) のロード失敗。 ${err.message}`);
    }
}
...
export function initializeCvObjects() {
    ...
    if (!ui.video.videoWidth || !ui.video.videoHeight) throw new Error("Video dimensions not available.");
...
export function detectFaces() {
    if (!cvReady || !cascadeReady || !cap || !src || !gray || !faces || src.isDeleted() || gray.isDeleted() || faces.isDeleted()) {
        console.warn("OpenCV objects not ready or deleted, skipping face detection.");
        return null;
    }
...
export function cleanupCvResources(clearCascade = true) {
    ...
    if (src && !src.isDeleted()) { src.delete(); src = null; console.log("src Mat deleted."); }
    if (gray && !gray.isDeleted()) { gray.delete(); gray = null; console.log("gray Mat deleted."); }
    if (faces && !faces.isDeleted()) { faces.delete(); faces = null; console.log("faces RectVector deleted."); }
...
async function createFileFromUrl(pathInFS, url) {
    try {
        ...
        if (!response.ok) {
            if (response.status === 404) console.error(`File not found at ${url}. Check path.`);
